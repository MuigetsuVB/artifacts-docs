## Update 29/06/25: Season 5 (Draft)
...
- Badges are now automatically granted when the corresponding condition is met.
...
- Aside from the default skins, unlocked skins are visible through the Get Account endpoint (`/accounts/{account}`).
 ...
 **Combat**
- The "attack block" system for monsters and players has been removed. It was too random and disconnected from gameplay.
...
**Bank**
- Fixed a bug where depositing items into the bank generated two log messages.
...
- Hell gear moved to level 45 and new gear added.
- Some items have been reworked (name, image) to better match the Artifacts universe.
...
- It is now possible to give items or gold to another one of your characters on the same map using two new endpoints:
  - Give Gold: `/my/{name}/action/give_gold`
  - Give Item: `/my/{name}/action/give_item`
