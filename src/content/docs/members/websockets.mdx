...
:::caution
WebSockets is a feature reserved for members. You can sign up for a subscription in your [member area](https://www.artifactsmmo.com/account/).
:::
...
| Raid started|raid_started| Raids |Server| Send `{ result: null, raid: RaidSchema }` when a raid instance starts. |
| Raid ended|raid_ended| Raids |Server| Send `{ result: RaidInstanceResult, raid: RaidSchema }` when a raid instance ends. `RaidInstanceResult` is `success` or `failure`. |
...
| Grand Exchange cancel sell order | grandexchange_cancel_sell_order | Grand Exchange | Server | Send `GEOrderSchema` when a sell order is cancelled on the Grand Exchange |
| Grand Exchange cancel buy order | grandexchange_cancel_buy_order | Grand Exchange | Server | Send `GEOrderSchema` when a buy order is cancelled on the Grand Exchange |
...
| Announcement |announcement | Announcements|Server| Sends an announcement message when a new announcement is published. |
...
| Season reward unlocked|season_reward_unlocked| Season Rewards |Account| Send `SeasonRewardSchema` to the targeted account when a season reward is granted. |
...
message = {
        "token": token,
        "subscriptions": ["event_spawn", "event_removed", "grandexchange_buy_order", "grandexchange_sell_order", "achievement_unlocked", "test"]
    }    
```
...
message = {
        "token": token,
        "subscriptions": ["event_spawn", "event_removed", "grandexchange_buy_order", "grandexchange_sell", "achievement_unlocked", "test"]
    }    
...
const message = {
    token: token,
    subscriptions: ["event_spawn", "event_removed", "grandexchange_buy_order", "grandexchange_sell"]
};
...
public class WebSocketHandler extends WebSocketClient {
    ...
    public void onOpen(ServerHandshake handshakedata) {
        String message = """
        {"token": "%s",
        "subscriptions": ["event_spawn", "event_removed,"grandexchange_buy_order", "grandexchange_sell", "achievement_unlocked"]};
        """.formatted(TOKEN);
        send(message);
