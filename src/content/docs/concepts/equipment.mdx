...
Votre personnage possède 15 emplacements d'équipement qui vous permettrent de modifier vos statistiques et d'ajouter des effects uniques à votre personnage.

## Equipment Types

Voici les catégories d'équipements disponibles dans le jeu :
...
Les items utilisent un système d'effects qui permettent de modifier les statistiques du personnage. Il existe un tas d'effects dont les effects de base qui permettrent de gagner des statistiques mais aussi des effects plus complexe pour les utilites et les runes.

Voici des exemples d'équipements avec leurs effets :
...
Voici des exemples d'utilitaires avec leurs effets :
...
Voici des exemples de runes avec leurs effets :
...
Pour obtenir une liste paginée de tous les objets, utilisez le point de terminaison suivant. Le filtre `type` est particulièrement utile pour trouver tous les équipements d'une catégorie spécifique. Par exemple, pour récupérer toutes les armes :
...
with ArtifactsClient() as client:
    weapons = client.items.get_all(type="weapon")
  ```
  </TabItem>
</Tabs>
