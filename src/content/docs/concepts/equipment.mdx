...
Your character has 16 equipment slots that allow you to modify your stats and add unique effects to your character.
...
To equip one or more items when you meet the conditions, use the following endpoint. The request body is an array containing between 1 and 20 items. The cooldown is **3 seconds per array entry**.
...
const options = {
  ...
  body: JSON.stringify([
    ...
    { code: "iron_armor", slot: "body_armor" }
  ])
};
...
:::caution
Your character must have enough inventory space for the returned items and enough HP to survive any stat changes from unequipping.
:::

To unequip one or more items, use the following endpoint. The request body is an array containing between 1 and 20 items. The cooldown is **3 seconds per array entry**.
