...
Here are all the stats a character can have.  
...
**Example:** A standard monster response.
```http title="JSON"
{
  "data": {
    "name": "Spider",
    ...
    "effects": [
      {
        "code": "poison",
        "value": 20,
        "description": "At the start of its first turn, applies a 20 poison to its opponent. Loses 20 HP per turn."
      }
    ],
    "min_gold": 0,
    "max_gold": 10,
...
In the example above, the Spider has the `poison` effect with a value of `20`. Here are all the possible effects you might encounter:
...
[View API Reference](https://api.artifactsmmo.com/docs/#/operations/get_all_monsters_monsters_get)
...
[View API Reference](https://api.artifactsmmo.com/docs/#/operations/get_monster_monsters__code__get)
