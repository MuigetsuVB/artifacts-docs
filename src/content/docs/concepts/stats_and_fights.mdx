...
| Sun Shield | Reduces the first direct hit taken between the monster's turns by X%. |
| Greed | Gains X% damage each time the bearer loses 10% of their max HP. |
| Enchanted Mirror | After taking damage, deals back X% of that damage to the attacker. Can trigger once every 3 turns. |
