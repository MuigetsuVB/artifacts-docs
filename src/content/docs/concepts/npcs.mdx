...
Voici un exemple pour afficher des informations sur des monstres :
...
with ArtifactsClient(token="YOUR_TOKEN") as client:
    items = client.npcs.get_items("{npc_code}")
    for item in items.data:
        print(f"{item.code} buy={item.buy_price} sell={item.sell_price}")
  ```
  </TabItem>
</Tabs>
...
- [Maps & Movement](/concepts/maps_and_movement/) — Finding NPC locations on the map
- [Equipment](/concepts/equipment) — Items you can buy and sell
- [Tasks](/concepts/tasks/) — Accepting tasks from task masters
- [Grand Exchange](/concepts/grand_exchange/) — Player-to-player trading alternative
